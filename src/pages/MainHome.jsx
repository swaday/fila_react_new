...
function MainHome({ setCurrentPage }) {
    ...
    const MODAL_PRODUCTS = [
        { id: 'product1', name: '에샤페 벨크로 메리제인 초코', color: 'BROWN, BROWN, BROWN', price: 99900, image: './image/item1.webp' },
        { id: 'product2', name: '에샤페 실버문', color: 'GRAY, WHITE, WHITE', price: 109000, image: './image/item2.webp' },
        { id: 'product3', name: '에샤페 LX 크림', color: 'WHITE, BROWN, WHITE', price: 119000, image: './image/item3.webp' },
    ];

    const [modalChecked, setModalChecked] = useState({ product1: false, product2: false, product3: false });

    const modalAllChecked = Object.values(modalChecked).every(Boolean);

    const handleModalAllCheck = (e) => {
        const next = e.target.checked;
        const updated = {};
        MODAL_PRODUCTS.forEach(p => { updated[p.id] = next; });
        setModalChecked(updated);
    };

    const handleModalItemCheck = (id) => {
        setModalChecked(prev => ({ ...prev, [id]: !prev[id] }));
    };

    const modalTotalPrice = MODAL_PRODUCTS.filter(p => modalChecked[p.id]).reduce((sum, p) => sum + p.price, 0);
                ...
                <div className="modal">
                    ...
                    <section className="form_section">
                                        ...
                                        <div className="product_item">
                                            <label className="product_item_label">
                                                <input
                                                    type="checkbox"
                                                    className="common-checkbox"
                                                    checked={modalChecked[p.id]}
                                                    onChange={() => handleModalItemCheck(p.id)} />
                        ...
                        <div className="total_price_container">
                            <b>주문금액</b>
                            <b>{modalTotalPrice.toLocaleString()}원</b>
                        </div>
                        <div className="submit_container">
                            <button type="button" disabled={modalTotalPrice === 0}>장바구니 담기</button>
                        </div>
                    </section>
                </div>
            </div>
